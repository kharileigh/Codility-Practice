...
package com.kharileigh.tieropes;
...
public class TieRopes {
    ...
    // Find number of ropes whose length is greater than or equal to K is maximal
    public int solution(int K, int[] A) {
        ...
        // 2 : FIND ADJACENT ROPES - only adjacent can be tied
        for (int i = 0; i < A.length; i++) {
        
            currentLength = currentLength + A[i];
            if (currentLength >= K) {
            
                // increment value when rope found & reset current length each loop
                totalRopes++;
                currentLength = 0;
            }
        }
